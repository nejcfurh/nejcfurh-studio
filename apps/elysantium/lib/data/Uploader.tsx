...
import { bookings } from '@/lib/data/data-bookings';
import { cabins } from '@/lib/data/data-cabins';
...
async function createBookings(): Promise<void> {
  ...
  const { data: guestsIds } = await supabase
    ...
    .select('id')
    .order('id');
  const allGuestIds: number[] = guestsIds!.map(
    (cabin: { id: number }) => cabin.id
  );
  const { data: cabinsIds } = await supabase
    .from('cabins')
    .select('id')
    .order('id');
  const allCabinIds: number[] = cabinsIds!.map(
    (cabin: { id: number }) => cabin.id
  );
  ...
  const finalBookings = bookings.map((booking) => {
    ...
    const numNights: number = subtractDates(booking.endDate, booking.startDate);
    const cabinPrice: number =
      numNights * (cabin.regularPrice - cabin.discount);
    const extrasPrice: number = booking.hasBreakfast
      ? numNights * 15 * booking.numGuests
      : 0; // hardcoded breakfast price
    const totalPrice: number = cabinPrice + extrasPrice;
